...
def plot_results(results, directory, random_counterpart=None, random_concepts=None, num_random_exp=100,
    min_p_val=0.05):
  ...
  fig.tight_layout()
  plt.show()
  full_path = os.path.join(directory, "tcav_result.jpg")
  plt.savefig(full_path)

# Replace the code to display the results with code to write results to a CSV file
def write_results_to_csv(results, csv_file):
    with open(csv_file, 'w', newline='') as csvfile:
        fieldnames = ['Class', 'Concept', 'Bottleneck', 'TCAV_Score_Mean', 'TCAV_Score_Std', 'Random_Mean', 'Random_Std', 'P_Value', 'Significance']
        ...
        for result in results:
            ...
            writer.writerow({
                'Class': results[0]['target_class'],
                'Concept': concept,
                'Bottleneck': bottleneck,
                'TCAV_Score_Mean': np.mean(i_ups),
                'TCAV_Score_Std': np.std(i_ups),
                'Random_Mean': np.mean(random_i_ups[bottleneck]),
                'Random_Std': np.std(random_i_ups[bottleneck]),
                'P_Value': p_val,
                'Significance': 'not significant' if p_val > min_p_val else 'significant'
            })