...
class ModelWrapper(six.with_metaclass(ABCMeta, object)):
  ...
  def __init__(self, model_path=None, node_dict=None):
    ...
    Args:
      model_path: one of the following: 
      1) Directory path to checkpoint 
      2)Directory path to SavedModel 
      3) File path to frozen graph.pb 
      4) File path to frozen graph.pbtxt
      ...
      with self.sess.graph.as_default():
        if tf.io.gfile.isdir(model_path):
          ...
          if ckpt:
            ...
            #print('Loading from SavedModel dir.')
            tf.compat.v1.saved_model.loader.load(self.sess, ['serve'], model_path)
        else:
          input_graph_def = tf.compat.v1.GraphDef()
          if model_path.endswith('.pb'):
            #print('Loading from frozen binary graph.')
            with tf.io.gfile.GFile(model_path, 'rb') as f:
              input_graph_def.ParseFromString(f.read())
          else:
            #print('Loading from frozen text graph.')
  ...
  def _find_ends_and_bottleneck_tensors(self, node_dict):
    ...
    for k, v in six.iteritems(node_dict):
      if self.import_prefix:
        v = 'import/' + v
      #[Jihen ] Returns the Operation with the given name.
  ...
  def _make_gradient_tensors(self):
    ...
    #print(' I am in _make_gradient_tensors ')
    #print('here is the bottlenecks_tensors')
    ...
    for bn in self.bottlenecks_tensors:
      ...
      #print('self.bottlenecks_gradients[bn]')
      #print(self.bottlenecks_gradients[bn])
  ...
  def run_examples(self, examples, bottleneck_name):
    ...
    #print('I am in run_examples')
    #print(bottleneck_name)
    #print(examples)
    #print(self.bottlenecks_tensors[bottleneck_name])
    '''
    self.ends[input]examples
{<tf.Tensor 'Placeholder:0' shape=(?, ?, ?, 3) dtype=float32>:
 array([[[[0.99215686, 0.81960785, 0.6745098 ],
         [0.9882353 , 0.84705883, 0.6901961 ],
         [0.9764706 , 0.84705883, 0.6784314 ],
         ...,
    '''
    print('empty graph?')
    print(self.sess.graph.version)
    #0 why?
    #print('self.ends[input]examples')
   # print({self.ends['input']: examples})
...
class PublicImageModelWrapper(ImageModelWrapper):
  ...
  def __init__(self, sess, model_fn_path, labels_path, image_shape,
    ...
    #print('this is the self.ends')
    #print(self.ends)
    self.bottlenecks_tensors = PublicImageModelWrapper.get_bottleneck_tensors(
        scope)
    #print('this is the bottleneck tensor for publicimagemodelwrapper')
    #print(self.bottlenecks_tensors)
    ...
    with graph.as_default():
      ...
      self.loss = tf.reduce_mean(
          input_tensor=tf.compat.v1.nn.softmax_cross_entropy_with_logits_v2(
              ...
              logits=self.pred))
      #print('this is the self.loss of googlenet')
      #print (self.loss)
...
class GoogleNetWrapper_public(PublicImageModelWrapper):
  ...
  def __init__(self, sess, model_saved_path, labels_path):
    ...
    #in BGR order. The BGR mean values need to be subtracted as follows:
     #[104.0,117.0,123.0] before passing the image blob into the network.
    image_shape_v1 = [224, 224, 3]
    self.image_value_range = (-117, 255 - 117)
    #[Jihen] fill in self.ends dictionary with input, logit and prediction tensors
    endpoints_v1 = dict(
        input='input:0',
...
#[Jihen] class Cifairwrapper_model is added by me to create a wrapper for the trained model on cifair data
class CifairWrapper_model(ModelWrapper):
  ...
  def __init__(self, sess, model_saved_path, labels_path):
    ...
    self.sess = sess
    super(CifairWrapper_model, self).__init__()
    self.model_name = 'cifair_sequential'
...
class KerasModelWrapper(ModelWrapper):
  ...
  def __init__(
    ...
    self.labels = tf.io.gfile.GFile(labels_path).read().splitlines()
    # [Jihen] shape of the input image in this model
    self.image_shape = image_shape
    ...
    # [Jihen] added by me following this issue https://github.com/tensorflow/tcav/issues/81
  def get_image_shape(self):
    """returns the shape of an input image."""
    return self.image_shape
  ...
  def get_inputs_and_outputs_and_ends(self):
    ...
    #print('self.ends[prediction]')
    #print(self.ends['prediction'])

class CustomPublicImageModelWrapper(ImageModelWrapper):

    def __init__(self, sess, model_path, labels_path, image_shape,
                 name, image_value_range):

        super(CustomPublicImageModelWrapper, self).__init__(image_shape)
        ...
        #print('input tensor')
        #print(endpoints_dict['input_tensor'])
    ...
    def get_bottleneck_tensors():
        ...
        for op in graph.get_operations():
            ...
            if 'ConcatV2' in op.type:
                ...
                bn_endpoints[name] = op.outputs[0]
        return bn_endpoints
class KerasWrapper_public(CustomPublicImageModelWrapper):

  def __init__(self, sess, model_saved_path, labels_path):
    model = tf.keras.models.load_model(model_saved_path)

    image_value_range = (-1, 1)
    image_shape_v3 = [224, 224, 3]
    endpoints_v3 = dict(
    input=model.inputs[0].name,
    input_tensor=model.inputs[0],
    logit=model.outputs[0].name,
    ...
    self.sess = sess
    super(KerasWrapper_public, self).__init__(
        sess,
        labels_path,
        image_shape_v3,
        endpoints_v3,
